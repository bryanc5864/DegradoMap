...
The improved model achieves a multi-seed mean of 0.646\,\spm{0.124} on target-unseen evaluation---a 6.4\% improvement over the gradient boosting baseline (0.607).
...
For deployment, we recommend model ensembling across multiple seeds to achieve reliable performance closer to the average.
Figure~\ref{fig:multiseed_variance} visualizes the seed-to-seed variation.
...
\caption{Multi-seed validation reveals high variance (std=0.124). Individual seeds range from 0.506 to 0.745; mean (0.646) exceeds gradient boosting baseline (0.607) but not statistically significant (bootstrap p=0.259).}
...
This likely reflects two factors: (1)~training distribution bias (CRBN: 62\% of samples vs.\ VHL: 35\%), and (2)~fundamental biological differences in E3-target compatibility.
Notably, the E3-unseen evaluation (CRBN$\to$VHL: 0.811 AUROC) demonstrates successful transfer when VHL is the \emph{test E3 ligase} (i.e., predicting VHL degradation from CRBN training data), but this does not translate to target-unseen generalization for VHL-targeted proteins.
For rare E3 ligases (cIAP1, DCAF16, MDM2, etc.), sample counts are too small ($n{<}50$) for reliable evaluation.
Our evidence supports transfer primarily between the two major ligases (CRBN and VHL), not broad generalization across all E3 ligases.
Extending to rare E3s requires substantially more training data.
...
This calibration quality, combined with the modest but stable AUROC, suggests that \ours{} provides \emph{reliable} predictions even if not perfect discriminative performance.
...
The multi-seed target-unseen AUROC of 0.646\,\spm{0.124} (best: 0.7449) represents meaningful signal but reveals limits of structure-based prediction.
DegradeMaster~\cite{liu2025degrademaster}, using full PROTAC descriptors, achieves 0.856; the ${\sim}0.11$ gap quantifies PROTAC feature value.
The paradigms are complementary---\ours{} screens targets before PROTAC synthesis; full-information methods optimize existing designs.

The CRBN$\to$VHL E3-unseen performance (0.811 AUROC) demonstrates successful transfer between major ligases.
However, per-E3 target-unseen performance is asymmetric (CRBN: 0.758, VHL: 0.396), and rare E3 ligases lack sufficient data for reliable evaluation.
...
(1)~\textbf{Small dataset}: Only 155 unique proteins limit target-unseen generalization and contribute to high seed variance.
(2)~\textbf{E3 imbalance}: CRBN/VHL dominate (97\%); rare E3 generalization unsupported.
(3)~\textbf{Validation-test mismatch}: Validation AUROC does not reliably predict target-unseen test AUROC, as validation may share protein similarity with training (closer to interpolation than extrapolation). Ideally we would use target-unseen validation, but this would further reduce limited training data.
(4)~\textbf{No structural validation}: Predictions rely on static AlphaFold structures without physics-based validation (docking, MD, free energy calculations).
(5)~\textbf{No external validation}: PROTAC-8K is the only available benchmark; real-world generalization unvalidated.
(6)~\textbf{High seed variance}: std=0.124 may require ensembling for reliable deployment.
...
The model achieves 0.646\,\spm{0.124} AUROC (best seed: 0.7449) on unseen targets, 0.811 on CRBN$\to$VHL E3-unseen transfer, and 74\% Hit@3 for E3 recommendation.
Two findings carry broader implications: invariant architectures outperform equivariant ones for scalar prediction, and ESM-2 embeddings require task-specific regularization.
