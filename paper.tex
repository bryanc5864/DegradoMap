...
VHL-targeted proteins have similar positive rates to CRBN (36.9\% vs.\ 38.4\%), ruling out class imbalance.
The failure likely reflects structural heterogeneity: VHL substrates span diverse families with varying topology, while CRBN recognition relies on surface-exposed lysines adjacent to the E3 interface---a constraint our lysine pooling explicitly encodes.
\textbf{This E3-specificity is a key limitation}: practitioners should expect near-random performance for VHL-targeted and rare E3 proteins.
Table~\ref{tab:per_e3_breakdown} details the per-E3 breakdown.
...
To validate biological relevance, we analyze BRD4 (UniProt: O60885), a well-characterized CRBN PROTAC target~\cite{zengerle2015selective}.
\ours{} predicts high degradability (score: 0.87) and assigns top attention weights to K374 (weight: 0.12, SASA: 94\,\AA$^2$, inter-bromodomain linker), K311 (0.09), and K434 (0.08)---all surface-exposed residues.
Mass spectrometry identifies K374 as a major ubiquitination site following PROTAC treatment~\cite{zengerle2015selective}, corroborating the model's ranking.
This single case supports the biological grounding of lysine-weighted pooling; systematic multi-target validation remains future work.
