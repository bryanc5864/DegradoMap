...
This variance, consistent with EGNN baselines (0.518\,\spm{0.11}), reflects optimization challenges in the low-data regime.
Per-E3 analysis reveals asymmetric performance: CRBN targets achieve 0.758 AUROC ($n{=}273$) while VHL targets yield only 0.396 ($n{=}187$), likely reflecting both training distribution (CRBN: 62\%) and biological differences.
...
Additional analyses (5-fold CV, case studies) are in the Appendix.
...
The strong E3-unseen performance (0.811 AUROC for CRBN$\to$VHL) demonstrates effective cross-E3 transfer, though generalization to rare E3 ligases (cIAP1: 0.271 AUROC) requires more data.
Feature ablation reveals that removing the lysine indicator \emph{improves} AUROC (+0.101), suggesting domain knowledge is better encoded through architectural biases (lysine-weighted pooling) than input features.

\paragraph{Limitations.}
(1)~Dataset dominated by CRBN/VHL (96\%), limiting rare-E3 generalization.
(2)~Static AlphaFold structures ignore conformational dynamics.
(3)~No compatible external validation dataset exists.
(4)~High seed variance (std=0.124) may require ensembling for reliable peak performance.
...
The model achieves 0.646\,\spm{0.124} AUROC (best: 0.7449) on unseen targets, 0.811 on E3-unseen transfer, and 74\% Hit@3 for E3 recommendation.
