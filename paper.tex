...
Code, pre-trained models, and reproducibility scripts are available at \url{https://github.com/anonymoussubmitter-167/DKO}.
The PROTAC-8K dataset is available at \url{https://zenodo.org/records/14715718}.

\paragraph{Reproducibility Statement.}
To ensure reproducibility, we provide the complete source code, pre-trained model checkpoints, and all scripts necessary to reproduce our experiments at \url{https://github.com/anonymoussubmitter-167/DKO}.
The repository includes: (1)~model architecture implementations in PyTorch; (2)~data preprocessing pipelines for AlphaFold structures, ESM-2 embeddings, and PhosphoSitePlus annotations; (3)~training scripts with exact hyperparameter configurations; (4)~evaluation scripts for all reported metrics; and (5)~a Dockerfile for environment reproduction.
All experiments were conducted on NVIDIA RTX 2080 Ti GPUs (11\,GB).
We report multi-seed validation results (seeds 42, 123, 456) with mean and standard deviation to characterize variance.
The PROTAC-8K dataset is publicly available on Zenodo.
Random seeds, data splits, and hyperparameters are fully documented in the supplementary materials.
